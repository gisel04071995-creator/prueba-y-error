...
function renderizarTablaTurnos() {
  ...
  turnos.forEach(t => {
    ...
    tr.innerHTML = `
      ...
      <td>
        <button class="btn btn-sm btn-editar me-1" onclick="window.editarTurno(${t.id})">Editar</button>
        <button class="btn btn-sm btn-eliminar" onclick="window.eliminarTurno(${t.id})">Eliminar</button>
      </td>
    `;
